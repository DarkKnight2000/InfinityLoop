module Main where
    ...
    eachGrid pics x y a bgc = translate (x*gridSize'+x-2*gridSize') (y*gridSize'+y-2*gridSize') $ color bgc $ pics where
    ...
    pRenderState :: [Picture] -> [Int] -> Float -> Float -> Float -> Float -> Float -> Float -> Int -> Int -> [Char] -> [Picture]
    pRenderState pic [] x y r c px py mov bes capt = [ translate (-500) 200 $ scale 0.5 0.5 $ color white $ text ("Moves : " ++ (show mov))] ++ [ translate (-300) 300 $ scale 0.5 0.5 $ color white $ text capt] ++ [ translate (-350) (-300) $ scale 0.2 0.2 $ color white $ text "Press Up arrow to move up"] ++ [ translate (-350) (-330) $ scale 0.2 0.2 $ color white $ text "Press Down arrow to move down"] ++ [ translate (-350) (-360) $ scale 0.2 0.2 $ color white $ text "Press Left arrow to move left"] ++ [ translate (-350) (-390) $ scale 0.2 0.2 $ color white $ text "Press Right arrow to move right"] ++ [ translate (-350) (-420) $ scale 0.2 0.2 $ color white $ text "Press Enter key to rotate a piece"]
    pRenderState pic (a:xs) x y r c px py mov bes capt = (if x == (toFloat (floor px) 0) && y == (toFloat (floor py) 0) then ([eachGrid (pic!!(a+5)) x y a green]++[eachGrid high x y a white]) else ([eachGrid (pic!!(a+5)) x y a black])) ++ (pRenderState pic xs (fst (getNextPos x y r c)) (snd (getNextPos x y r c)) r c px py mov bes capt)
    ...
    renderState a s =
        ...
        else if tempStage == 4 then pictures([a!!3]++[translate 200 (-125) $ scale 0.5 0.5 $ color white $ text (show (moves s))])
        else if tempStage == 7 then a!!4
        else pictures((pRenderState a (grids s) 1 1 (toFloat (rows s) 0) (toFloat (cols s) 0) (posx s) (posy s) (moves s) (best s) (caption s))) where
            tempStage = (stage s)
    ...
    updateState s m =
        if st == 5 then m{crctConfig = temp1, grids = temp2 , randGen = temp3, stage = 3.4, caption = "Bonus Stage"}
    ...
    handleInput (EventKey(Char '2') (Up)_ _)game =  
        ...
        else if stage game == 3 then level32
        else game
